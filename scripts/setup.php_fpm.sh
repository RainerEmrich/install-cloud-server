...
setup_php_fpm () {
	...
	if [ "${PHP_FPM_INSTALLED}" != "1" ] ; then
		...
		sed --in-place 's/#Include \/etc\/apache2\/conf-available\/php7.0-fpm/Include \/etc\/apache2\/conf-available\/php7.0-fpm/' /etc/apache2/sites-available/${MY_SITE_CONFIG}.conf
